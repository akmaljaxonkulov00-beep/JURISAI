-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION: Fix Article Sorting + Backfill Users
-- ═══════════════════════════════════════════════════════════════════════════

-- ── PHASE 1: Add article_number_int column ───────────────────────────
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ── PHASE 2: Backfill auth.users → registered_users ──────────────────
...
-- ── PHASE 3: Ensure sync trigger exists ──────────────────────────────
CREATE OR REPLACE FUNCTION public.sync_auth_user_to_registered()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.registered_users (id, email, full_name, role, is_active, created_at)
  VALUES (
    NEW.id,
...
-- ── PHASE 4: Fix RLS policies to allow admin reads ───────────────────
