...
import { Crown, Star, Check, ArrowRight, Sparkles, Shield, Zap, Headphones, RefreshCw } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-16">
          {plans.map((plan, index) => {
            ...
            return (
            <Card key={index} className={`card-default relative ${isPopular ? 'ring-2 ring-blue-500 shadow-2xl transform scale-105' : 'shadow-lg'}`}>
              {isPopular && (
              ...
              <CardHeader className="text-center pb-4">
                ...
                <div className="mb-4">
                  <div className="text-4xl font-bold text-gray-900 dark:text-white">
                    ...
                    {!isFree && <span className="text-lg text-gray-500 dark:text-zinc-400 font-normal">/oyiga</span>}
                  </div>
                  {plan.caseLimit === -1 && (
                    <Badge className="mt-2 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">Cheksiz</Badge>
                  )}
                </div>
              </CardHeader>
              <CardContent className="pt-0">
                ...
                <Link href={`/manual-payment?plan=${plan.id}&amount=${plan.price}`}>
                  <Button className={`w-full ${isPopular ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-800 dark:bg-gray-600 hover:bg-gray-700 dark:hover:bg-gray-50 dark:bg-zinc-800/500'}`} size="lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {benefits.map((benefit, index) => (
              <Card key={index} className="card-default text-center p-6">
                ...
                <p className="text-gray-500 dark:text-gray-400 dark:text-zinc-500">{benefit.description}</p>
              </Card>
            ))}
          </div>
        </div>
