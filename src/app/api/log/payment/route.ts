...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, userEmail, userName, plan, amount, receiptImage } = body;

    if (!userEmail || !plan || !amount) {
      return NextResponse.json({ success: false, error: 'Email, plan and amount are required' }, { status: 400 });
    }
