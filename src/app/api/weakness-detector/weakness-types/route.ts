...
export async function GET(request: NextRequest) {
  try {
    ...
    const mockWeaknessTypes = [
      {
        ...
        description:
          "Argumentda mantiqiy xatoliklar mavjud - sabab va oqibat o'rtasida to'g'ri bog'lanish yo'q",
        severity: 'medium',
        examples: [
      ...
      },
      {
        ...
        detection_indicators: ['Barcha', 'Hech qachon', 'Har doim', 'Hech kim'],
        improvement_suggestions: [
    ...
    return NextResponse.json({
      ...
      summary: {
        most_common: mockWeaknessTypes.reduce((prev, current) =>
          prev.frequency > current.frequency ? prev : current
        ),
        most_severe: mockWeaknessTypes
          .filter(t => t.severity === 'high')
          .reduce((prev, current) => (prev.frequency > current.frequency ? prev : current)),
        easiest_to_fix: mockWeaknessTypes
