import { NextRequest, NextResponse } from 'next/server'

async function getSupabase() {
  const { createClient } = await import('@supabase/supabase-js')
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
  const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY
  if (!supabaseUrl || !supabaseKey) return null
  return createClient(supabaseUrl, supabaseKey)
}

// GET /api/community/groups/posts?groupId=...
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const groupId = searchParams.get('groupId')

    if (!groupId) {
      return NextResponse.json(
        { success: false, error: 'groupId kiritilishi shart' },
        { status: 400 }
      )
    }

    const supabase = await getSupabase()
    if (!supabase) {
      return NextResponse.json({ success: true, data: [] })
    }

    const { data, error } = await supabase
      .from('community_group_posts')
      .select('*')
      .eq('group_id', groupId)
      .order('created_at', { ascending: false })
      .limit(100)

    if (error) throw error

    return NextResponse.json({ success: true, data: data || [] })
  } catch (err: any) {
    return NextResponse.json(
      { success: false, error: err.message || 'Postlarni yuklashda xatolik' },
      { status: 500 }
    )
  }
}

// POST /api/community/groups/posts  { groupId, userId, userName, content }
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { groupId, userId, userName, content } = body

    if (!groupId || !content?.trim()) {
      return NextResponse.json(
        { success: false, error: 'groupId va content kiritilishi shart' },
        { status: 400 }
      )
    }

    const supabase = await getSupabase()
    if (!supabase) {
      return NextResponse.json(
        { success: false, error: 'Supabase sozlanmagan' },
        { status: 500 }
      )
    }

    const { data, error } = await supabase
      .from('community_group_posts')
      .insert({
        group_id: groupId,
        user_id: userId || '',
        user_name: userName || 'Foydalanuvchi',
        content: content.slice(0, 2000),
      })
      .select()
      .single()

    if (error) throw error

    // Guruh postlar sonini oshirish (joriy qiymatdan +1)
    try {
      const { data: grp } = await supabase
        .from('community_groups')
        .select('post_count')
        .eq('id', groupId)
        .single()
      await supabase
        .from('community_groups')
        .update({ post_count: (grp?.post_count || 0) + 1, updated_at: new Date().toISOString() })
        .eq('id', groupId)
    } catch {}

    return NextResponse.json({ success: true, data })
  } catch (err: any) {
    return NextResponse.json(
      { success: false, error: err.message || 'Post yozishda xatolik' },
      { status: 500 }
    )
  }
}
