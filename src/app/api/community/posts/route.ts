...
interface CommunityPost {
  id: string;
  author: { id: string; name: string; avatar: string; role: string; verified: boolean; reputation: number; };
  content: string;
  category: string;
  tags: string[];
  likes: number; dislikes: number; likedBy: string[]; dislikedBy: string[];
  comments: any[];
  views: number; isPinned: boolean;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.content || !body.category) {
      return NextResponse.json({ success: false, error: 'Content and category are required' }, { status: 400 });
    }
