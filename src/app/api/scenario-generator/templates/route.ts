  ...
  try {
    ...
    if (difficulty_level) {
      filteredTemplates = filteredTemplates.filter(
        template => template.difficulty_level === difficulty_level
      )
    }
      ...
      summary: {
        by_type: {
          ...
          other: filteredTemplates.filter(
            t => !['civil', 'criminal', 'family', 'labor', 'property'].includes(t.scenario_type)
          ).length,
        },
        by_difficulty: {
        ...
        average_duration: Math.round(
          filteredTemplates.reduce((sum, t) => sum + t.duration_minutes, 0) /
