...
export async function POST(request: NextRequest) {
  try {
    ...
    const baseScenario =
      mockScenarios[scenario_type as string]?.[difficulty_level] || mockScenarios['civil']['medium']
    ...
    const generatedScenario = {
      ...
      participants: baseScenario.participants.slice(
        0,
        participants_count || baseScenario.participants.length
      ),
...
function getRelevantLaws(scenarioType: string, issue: string): string[] {
  const lawMap: Record<string, string[]> = {
    civil: [
      ...
      "O'zbekiston Respublikasi \"Nizolarni hal etish to'g'risida\" qonuni",
    ],
    criminal: [
