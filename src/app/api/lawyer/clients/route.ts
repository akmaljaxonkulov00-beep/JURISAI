...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Mijozlarni olishda xatolik yuz berdi' }, { status: 500 })
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { lawyerId, name, email, phone, address, caseType, initialDescription } = body
    ...
    if (existingClient) {
      return NextResponse.json({ error: 'Bu mijoz allaqachon mavjud' }, { status: 400 })
    }
    ...
    if (clientError) {
      ...
      return NextResponse.json({ error: 'Mijozni yaratishda xatolik yuz berdi' }, { status: 500 })
    }

    // Create initial case if provided
    if (caseType && initialDescription) {
      const { error: caseError } = await supabaseServer.from('client_cases').insert({
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { clientId, name, email, phone, address, status, notes } = body

    if (!clientId) {
      return NextResponse.json({ error: 'Mijoz ID si kerak' }, { status: 400 })
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete client
    const { error: deleteError } = await supabaseServer.from('clients').delete().eq('id', clientId)
