...
import {
  ...
  calculateConfidence,
  getMockDocument,
} from '@/lib/document-generation/utils'
...
async function generateDocumentWithOpenAI(documentType: string, details: any) {
  ...
  try {
    ...
    return {
      type: documentType,
      title:
        typeof getDocumentTitle === 'function'
          ? (getDocumentTitle as any)(documentType, details)
          : documentType,
      content,
      metadata: {
        ...
        confidence:
          typeof calculateConfidence === 'function' ? (calculateConfidence as any)(content) : 85,
      },
    }
  } catch (error) {
