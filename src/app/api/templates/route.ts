...
export async function GET(request: NextRequest) {
  try {
    ...
    if (supabase) {
      ...
      } catch { /* Supabase query failed, fallback to hardcoded */ }
    }

    // ── 2. Fallback to hardcoded data ──
    const templates = (templatesFromDb && templatesFromDb.length > 0) ? templatesFromDb : DOCUMENT_TEMPLATES;
    ...
    if (id) {
      ...
      if (results.length === 0) {
        ...
        if (!localT) return NextResponse.json({ success: false, error: 'Hujjat topilmadi' }, { status: 404 });
        results = [localT];
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !category || !content) {
      return NextResponse.json({ success: false, error: 'Nomi, kategoriyasi va matni majburiy' }, { status: 400 });
    }

    // Try Supabase first
    if (supabase) {
      const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
      const { data, error: err } = await supabase.from('document_templates').insert({
        slug,
        name,
        category,
      ...
      if (!err && data) {
        return NextResponse.json({ success: true, message: 'Hujjat yaratildi', template: mapFromDb(data) });
      }
    }
    ...
    return NextResponse.json({
      ...
      template: {
        ...
        content, lawRef: lawRef || '', format: format || 'TXT',
        size: `${Math.ceil(content.length / 1024)} KB`,
        downloads: 0, createdAt: new Date().toISOString().split('T')[0],
        tags: tags || [],
      },
    });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (supabase) {
      ...
      const { error: err } = await supabase.from('document_templates').update(updateData).eq('slug', id);
      if (!err) return NextResponse.json({ success: true, message: 'Hujjat yangilandi' });
    }

    return NextResponse.json({ success: true, message: 'Hujjat yangilandi', template: { id, ...body } });
  } catch (error) {
