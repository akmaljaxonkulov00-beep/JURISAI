...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'Case ID talab qilinadi' }, { status: 400 })
    }
    ...
    const mockCaseDetails = {
      ...
      description:
        "Ikki kompaniya o'rtasida shartnoma shartlarining buzilishi munosabati bilan yuzaga kelgan nizo. Sotuvchi tomonidan tovar sifatiga ko'ra talab qilinayotgan xususiyatlar bo'lmaganligi, shu sababli xaridor tomonidan shartnoma bekor qilinishi va to'langan pulning qaytarilishi talabi qonuniy asoslangan.",
