...
import { ArrowLeft, TrendingUp, Clock, Target, Award, Calendar, BarChart3, Activity, Zap, BookOpen, Users, CheckCircle, AlertTriangle, Loader2, Plus, Trash2, X, Save, RefreshCw } from 'lucide-react';
...
interface ActivityRecord {
  id: string;
  type: 'case_completed' | 'case_attempted' | 'ai_chat' | 'document_generated' | 'court_session' | 'quiz_completed' | 'manual_entry';
  title: string;
  description: string;
  xp: number;
  timestamp: string;
...
const DEFAULT_STATS: UserStatsData = {
  xp: 0, level: 1, completedCases: 0, totalCases: 0,
  weeklyProgress: 0, streak: 0, studyTime: 0, averageAccuracy: 0,
  achievements: 0, recentActivity: [],
};
...
export default function Statistics() {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'skills' | 'activity' | 'manual'>('overview');
  ...
  const [skillLevels, setSkillLevels] = useState<SkillLevels>({
    civil: 0, criminal: 0, labor: 0, family: 0, administrative: 0, procedural: 0,
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (storedStats) {
        ...
        setStats({
          xp: parsed.xp || 0, level: parsed.level || 1,
          ...
          streak: parsed.streak || 0, studyTime: parsed.studyTime || 0,
      ...
      try {
        ...
        if (apiStats?.data) {
          ...
          setStats(prev => ({
            ...prev, xp: apiData.xp ?? prev.xp, level: apiData.level ?? prev.level,
    ...
    try {
      ...
      if (supabaseUsers.length > 0) {
        ...
        const simUsers = [
          ...
          { name: 'Bobur M.', xp: Math.round(myXp * 0.9), level: Math.max(1, myLevel - 1), streak: 7 },
          { name: 'Madina K.', xp: Math.round(myXp * 0.65), level: Math.max(1, myLevel - 1), streak: 5 },
  ...
  useEffect(() => { if (stats.xp > 0) buildLeaderboard(); }, [stats.xp]);

  // ── Manual entry ──────────────────────────────────────────────
  const addManualEntry = () => {
    if (!manualTitle.trim()) return;
    const newActivity: ActivityRecord = {
      id: Date.now().toString(), type: manualType,
      title: manualTitle.trim(), description: manualDesc.trim() || "Qo'lda qo'shilgan yozuv",
      xp: manualXP, timestamp: new Date().toISOString(),
    };
    ...
    updatedStats.weeklyProgress = Math.min(100, Math.round(((stats.xp + manualXP) % WEEKLY_GOAL_XP) / WEEKLY_GOAL_XP * 100));

    const inc: Record<string, keyof SkillLevels> = {
      case_completed: 'civil', case_attempted: 'criminal', ai_chat: 'procedural',
      document_generated: 'civil', court_session: 'procedural', quiz_completed: 'criminal',
      manual_entry: 'civil',
    };
    ...
    setManualTitle(''); setManualDesc(''); setManualXP(10); setShowManualEntry(false);
  };
  ...
  const resetData = () => {
    if (confirm("Barcha statistik ma'lumotlarni o'chirishni xohlaysizmi?")) {
      ...
      setSkillLevels({ civil: 0, criminal: 0, labor: 0, family: 0, administrative: 0, procedural: 0 });
      setDaysActive({});
    }
  };
  ...
  const getFilteredActivity = () => {
    const now = Date.now();
    switch (timeFilter) {
      case 'week': return stats.recentActivity.filter(a => (now - new Date(a.timestamp).getTime()) < 7 * 24 * 60 * 60 * 1000);
      case 'month': return stats.recentActivity.filter(a => (now - new Date(a.timestamp).getTime()) < 30 * 24 * 60 * 60 * 1000);
      default: return stats.recentActivity;
    }
  };
  ...
  const getActivityIcon = (type: ActivityRecord['type']) => {
    switch (type) {
      case 'case_completed': return <CheckCircle className="w-4 h-4" />;
      case 'case_attempted': return <Activity className="w-4 h-4" />;
      case 'ai_chat': return <Zap className="w-4 h-4" />;
      case 'document_generated': return <BookOpen className="w-4 h-4" />;
      case 'court_session': return <Award className="w-4 h-4" />;
      case 'quiz_completed': return <Target className="w-4 h-4" />;
      default: return <Plus className="w-4 h-4" />;
    }
  };
  const getActivityColor = (type: ActivityRecord['type']) => {
    switch (type) {
      case 'case_completed': return 'text-green-600';
      case 'case_attempted': return 'text-orange-600';
      case 'ai_chat': return 'text-blue-600';
      case 'document_generated': return 'text-purple-600';
      case 'court_session': return 'text-indigo-600';
      default: return 'text-gray-600';
    }
  };
  const getActivityBg = (type: ActivityRecord['type']) => {
    switch (type) {
      case 'case_completed': return 'bg-green-50 dark:bg-green-900/20';
      case 'case_attempted': return 'bg-orange-50 dark:bg-orange-900/20';
      case 'ai_chat': return 'bg-blue-50 dark:bg-blue-900/20';
      case 'document_generated': return 'bg-purple-50 dark:bg-purple-900/20';
      case 'court_session': return 'bg-indigo-50 dark:bg-indigo-900/20';
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-zinc-800">
      ...
      <div className="min-h-screen bg-[#f8faff] dark:bg-gray-950 mobile-safe-top p-4 sm:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">Statistika</h1>
              <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-300">Platformadagi faoliyatingiz tahlili</p>
            </div>
          </div>
          <AnalysisError
    ...
    <div className="min-h-screen bg-[#f8faff] dark:bg-gray-950 mobile-safe-top">
      <div className="flex flex-col lg:flex-row">
        <div className="hidden lg:block w-64 bg-white dark:bg-zinc-900 border-r border-gray-100 dark:border-zinc-800 min-h-screen flex-shrink-0">
          <div className="p-6">
            <a href="/dashboard" className="flex items-center gap-3 px-3 py-2 text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 rounded-lg cursor-pointer mb-6 transition-colors">
            ...
            <nav className="space-y-2">
              <div className="flex items-center gap-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 rounded-lg">
                <BarChart3 className="w-5 h-5" /><span className="font-medium">Statistika</span>
              </div>
            </nav>
            <div className="mt-8">
              <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-bold text-gray-800 dark:text-zinc-100">{stats.xp.toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-green-600">{stats.completedCases}/{stats.totalCases || stats.completedCases}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <header className="bg-white dark:bg-zinc-900 px-4 sm:px-6 lg:px-8 py-4 border-b border-gray-100 dark:border-zinc-800">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex items-center gap-3">
                <a href="/dashboard" className="lg:hidden p-2 text-gray-600 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                {!apiConnected && (
                  <span className="text-[10px] px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full">Mahalliy ma'lumotlar</span>
                )}
                <div className="flex items-center gap-2 bg-orange-50 dark:bg-orange-900/30 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg">
                  ...
                  <span className="font-bold text-sm sm:text-base text-orange-600">{stats.streak}</span>
                ...
                <div className="flex bg-gray-100 dark:bg-zinc-800 rounded-lg p-0.5">
                  {[
                    ...
                    <button key={filter.value} onClick={() => setTimeFilter(filter.value)}
          ...
          <main className="p-3 sm:p-6 lg:p-8">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {[
                  { id: 'overview' as const, label: "Umumiy ko'rish", icon: <BarChart3 className="w-3.5 h-3.5" /> },
                  { id: 'skills' as const, label: 'Bilim darajasi', icon: <Target className="w-3.5 h-3.5" /> },
                  { id: 'activity' as const, label: 'Faollik', icon: <Activity className="w-3.5 h-3.5" /> },
                  { id: 'manual' as const, label: 'Faoliyat', icon: <BookOpen className="w-3.5 h-3.5" /> },
                ].map(view => (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        {stats.xp > 0 && <span className="text-xs text-green-600 font-medium">Faol</span>}
                      </div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">{stats.xp.toLocaleString()}</h3>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400 mt-0.5">Umumiy XP</p>
                    </div>
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        <span className="text-xs text-gray-500">{stats.completedCases}/{stats.totalCases || stats.completedCases}</span>
                      </div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">{stats.completedCases}</h3>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400 mt-0.5">Yechilgan ishlar</p>
                    </div>
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        <span className="text-xs text-green-600 font-medium">{stats.averageAccuracy > 0 ? `${stats.averageAccuracy}%` : '—'}</span>
                      </div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">{stats.averageAccuracy > 0 ? `${stats.averageAccuracy}%` : '0%'}</h3>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400 mt-0.5">O'rtacha aniqlik</p>
                    </div>
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-3 sm:mb-4">
                        ...
                        <span className="text-xs text-green-600 font-medium">{stats.streak} kun</span>
                      </div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">{Math.floor(stats.studyTime / 60)} soat</h3>
                      <p className="text-xs sm:text-sm text-gray-600 dark:text-zinc-400 mt-0.5">O'quv vaqti</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-sm text-gray-500 dark:text-zinc-400">{weeklyXP} / {WEEKLY_GOAL_XP} XP</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 dark:text-zinc-400 mt-1">
                      ...
                      <span>{weeklyGoalPct >= 100 ? "✅ Maqsad bajarildi!" : `${WEEKLY_GOAL_XP - weeklyXP} XP qoldi`}</span>
                    </div>
                    {weeklyGoalPct >= 100 && (
                  ...
                  {leaderboard.length > 1 && (
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-xs text-gray-400 dark:text-zinc-500">Platforma bo'ylab</span>
                      </div>
                      <div className="space-y-1.5">
                        {leaderboard.map((entry, i) => {
                          ...
                          return (
                            ...
                            >
                              <span className="w-6 text-center text-sm font-bold text-gray-400 dark:text-zinc-500">{medal}</span>
                              <div className="flex-1 min-w-0">
                                <p className={`text-sm font-medium truncate ${entry.isCurrentUser ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-zinc-100'}`}>
                      ...
                      <div className="space-y-2">
                        {(stats.recentActivity ?? []).slice(0, 5).map((a) => (
                          <div key={a.id} className="flex items-start gap-3 p-2 sm:p-3 bg-gray-50 dark:bg-zinc-800/50 rounded-lg">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-gray-800 dark:text-zinc-100 truncate">{a.title}</p>
                              <p className="text-xs text-gray-500 dark:text-zinc-400 truncate">{a.description}</p>
                              <p className="text-[10px] text-gray-400 dark:text-zinc-500 mt-0.5">
                                {new Date(a.timestamp).toLocaleDateString('uz-UZ', { day: 'numeric', month: 'short' })}
                              </p>
                            </div>
                            <span className="text-xs font-medium text-green-600 flex-shrink-0">+{a.xp} XP</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {stats.recentActivity.length === 0 && (
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-6 sm:p-8 text-center shadow-sm border border-gray-100 dark:border-zinc-800">
                      ...
                      <h3 className="font-bold text-gray-800 dark:text-zinc-100 mb-2">Hali faoliyat yo'q</h3>
                      <p className="text-xs sm:text-sm text-gray-500 dark:text-zinc-400 mb-4 max-w-md mx-auto">
                        Qonunlar bazasida keyslarni yeching, AI chatdan foydalaning yoki hujjatlar yarating.
                      </p>
                      <button onClick={() => setSelectedView('manual')}
                  ...
                  {stats.recentActivity.length > 0 && (
                    <div className="flex justify-between items-center">
                      <button onClick={() => setSelectedView('manual')}
                        ...
                        <Plus className="w-3.5 h-3.5" />Yangi yozuv
                      </button>
                      <button onClick={resetData}
                        ...
                        <Trash2 className="w-3.5 h-3.5" />Ma'lumotlarni tiklash
                      </button>
                    </div>
                  )}
                </div>
                  ...
                  <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {Object.entries(skillLevels).map(([skill, level]) => {
                        ...
                        return (
                          <div key={skill} className={`p-3 sm:p-4 rounded-xl ${hasActivity ? 'bg-gray-50 dark:bg-zinc-800/50' : 'bg-gray-50/50 dark:bg-zinc-800/20'}`}>
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-xs sm:text-sm font-medium text-gray-800 dark:text-zinc-100">{SKILL_NAMES[skill] || skill}</span>
                              <span className={`text-sm font-bold ${getSkillColor(level)}`}>{hasActivity ? `${level}%` : '—'}</span>
                            </div>
                            <div className="bg-gray-200 dark:bg-zinc-700 rounded-full h-2 sm:h-2.5">
                              <div className={`h-2 sm:h-2.5 rounded-full transition-all duration-500 ${
                                level >= 80 ? 'bg-green-500' : level >= 50 ? 'bg-yellow-500' : level >= 25 ? 'bg-orange-500' : 'bg-gray-300 dark:bg-zinc-600'
                              }`} style={{ width: `${Math.max(level, hasActivity ? 5 : 0)}%` }} />
                            </div>
                            {!hasActivity && <p className="text-[10px] sm:text-xs text-gray-400 dark:text-zinc-500 mt-1">Faoliyat boshlanmagan</p>}
                  ...
                  <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-zinc-800">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs text-gray-600 dark:text-zinc-400">
                        <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-gray-100 dark:bg-zinc-800 rounded" /><span>Yo'q</span></div>
                        <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-200 dark:bg-green-700/40 rounded" /><span>Kam</span></div>
                        <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-500 rounded" /><span>O'rtacha</span></div>
                        <div className="flex items-center gap-1"><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-600 rounded" /><span>Yuqori</span></div>
                      </div>
                    </div>
                    {Object.keys(daysActive).length > 0 ? (
                      <div className="space-y-1">
                        {Array.from({ length: Math.min(12, Math.ceil(Object.keys(daysActive).length / 7)) }, (_, rowI) => (
                          <div key={rowI} className="flex items-center gap-1.5 sm:gap-2">
                            <span className="text-[10px] sm:text-xs text-gray-500 dark:text-zinc-500 w-8 sm:w-10 flex-shrink-0">{rowI * 7}-{(rowI + 1) * 7} kun</span>
                            <div className="flex gap-0.5 sm:gap-1">
                              {Array.from({ length: 7 }, (_, colI) => {
                                const idx = rowI * 7 + colI;
                                const d = new Date(); d.setDate(d.getDate() - (83 - idx));
                                const key = d.toISOString().split('T')[0];
                                const level = daysActive[key] || 0;
                                return <div key={colI} className={`w-5 h-5 sm:w-7 sm:h-7 rounded ${getHeatmapColor(level)} border border-gray-100 dark:border-zinc-800`}
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-gray-500 dark:text-zinc-400">Faollik ma'lumotlari hali mavjud emas.</p>
                      </div>
                    )}
                    <div className="flex flex-wrap items-center justify-between gap-3 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200 dark:border-zinc-800">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        ...
                        <span className="text-[10px] sm:text-xs text-gray-600 dark:text-zinc-400">{Object.keys(daysActive).length} kun qayd etilgan</span>
                      </div>
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        <Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-600" />
                        <span className="text-[10px] sm:text-xs font-medium text-orange-600">{stats.streak} kun davomiylik</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => setShowManualEntry(!showManualEntry)}
                      ...
                      <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4" />Yangi yozuv
                    </button>
                  </div>
                  ...
                  {showManualEntry && (
                    <div className="bg-white dark:bg-zinc-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm border border-blue-200 dark:border-blue-800">
                      <h4 className="font-semibold text-gray-800 dark:text-zinc-100 mb-4 text-sm sm:text-base">Yangi faoliyat yozuvi</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4">
                        <div>
                          <label className="block text-xs text-gray-500 dark:text-zinc-400 mb-1">Faoliyat turi</label>
                          <select value={manualType} onChange={(e) => setManualType(e.target.value as ActivityRecord['type'])}
                        ...
                        <div>
                          <label className="block text-xs text-gray-500 dark:text-zinc-400 mb-1">XP miqdori</label>
                          <input type="number" value={manualXP}
                            onChange={(e) => setManualXP(Math.max(1, Math.min(1000, parseInt(e.target.value) || 0)))}
                            className="w-full px-3 py-2 bg-gray-50 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-lg text-xs sm:text-sm text-gray-800 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            min={1} max={1000} />
                        </div>
                        ...
                        <div className="sm:col-span-2">
                          <label className="block text-xs text-gray-500 dark:text-zinc-400 mb-1">Tavsif (ixtiyoriy)</label>
                      ...
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => setShowManualEntry(false)}
                          className="px-3 sm:px-4 py-2 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg text-xs sm:text-sm transition-colors">Bekor qilish</button>
                        <button onClick={addManualEntry} disabled={!manualTitle.trim()}
                          ...
                          <Save className="w-3.5 h-3.5" />Saqlash</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    {filteredActivity.length > 0 ? filteredActivity.map((a) => (
                      <div key={a.id} className={`flex items-start gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl ${getActivityBg(a.type)} transition-colors`}>
                        <div className={`mt-0.5 ${getActivityColor(a.type)}`}>{getActivityIcon(a.type)}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-zinc-100">{a.title}</p>
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-zinc-400">{a.description}</p>
                          <p className="text-[10px] text-gray-400 dark:text-zinc-500 mt-0.5">
                            {new Date(a.timestamp).toLocaleDateString('uz-UZ', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <span className="text-[10px] sm:text-xs font-medium text-green-600 dark:text-green-400">+{a.xp} XP</span>
                          <button onClick={() => deleteActivity(a.id)} className="p-1 text-gray-400 hover:text-red-500 transition-colors" title="O'chirish">
                            <X className="w-3 h-3" />
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="text-center py-8 sm:py-12">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500 dark:text-zinc-400 mb-4">
                          {timeFilter === 'week' ? "Bu hafta hech qanday faoliyat yo'q" : timeFilter === 'month' ? "Bu oy hech qanday faoliyat yo'q" : "Hali faoliyat qayd etilmagan"}
                        </p>
                        <button onClick={() => setShowManualEntry(true)}
                          className="px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg text-xs sm:text-sm hover:bg-blue-700 transition-colors">Birinchi yozuvni qo'shish</button>
                      </div>
                    )}
                  </div>
                </div>
