...
function FloatingCard({ children, depth = 0, index = 0, mouseX, mouseY, className = '' }: {
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.07, z: 45, transition: { type: 'spring', stiffness: 250, damping: 12 } }}
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: '0 8px 40px rgba(0,0,0,0.2), 0 2px 8px rgba(0,0,0,0.1), inset 0 1px 0 rgba(255,255,255,0.18)'
        }}
      >
        {/* Gradient border glow on hover */}
        <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
          style={{
            background: 'linear-gradient(135deg, rgba(59,130,246,0.08), rgba(99,102,241,0.05), transparent)',
          }}
        />
        {/* Hover glow ring */}
...
function ScaleIcon({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="M12 2v20M3 12h18" /><path d="M6 6l3 3M18 6l-3 3" />
      <circle cx="6" cy="10" r="1" /><circle cx="18" cy="10" r="1" />
...
const ECOSYSTEM_NODES: EcosystemNode[] = [
  { id: 'court', icon: <ScaleIcon className="w-4.5 h-4.5" />, title: 'Virtual Sud AI', description: 'Sud jarayonlarini simulyatsiya qilish', accent: '#06b6d4', depth: 5, x: 0, y: -168, floatAmp: 9, floatDur: 5.5, floatDelay: 0.2, pulseDelay: 0 },
  { id: 'gavel', icon: <BrainIcon className="w-4.5 h-4.5" />, title: 'AI Huquqiy Agent', description: "O'zR qonunchiligi asosida AI yordamchi", accent: '#6366f1', depth: 4, x: 146, y: -84, floatAmp: 7, floatDur: 6.2, floatDelay: 0.7, pulseDelay: 0.5 },
  { id: 'documents', icon: <FileTextIcon className="w-4.5 h-4.5" />, title: 'AI Hujjat Generator', description: "Da'vo va shartnomalarni avtomatik yaratish", accent: '#f59e0b', depth: 3, x: 146, y: 84, floatAmp: 8, floatDur: 5.8, floatDelay: 1.2, pulseDelay: 1.0 },
  { id: 'laws', icon: <BookIcon className="w-4.5 h-4.5" />, title: "O'zbekiston Qonunchiligi", description: 'Kodekslar va normativ hujjatlar bazasi', accent: '#10b981', depth: 6, x: 0, y: 168, floatAmp: 6, floatDur: 6.8, floatDelay: 1.7, pulseDelay: 1.5 },
  { id: 'search', icon: <SearchIcon className="w-4.5 h-4.5" />, title: 'Smart Huquqiy Qidiruv', description: "Sun'iy intellekt bilan semantik qidiruv", accent: '#8b5cf6', depth: 4, x: -146, y: 84, floatAmp: 10, floatDur: 5.3, floatDelay: 2.2, pulseDelay: 2.0 },
  { id: 'analytics', icon: <ChartIcon className="w-4.5 h-4.5" />, title: 'AI Analitika', description: "Huquqiy tahlil va bashoratli tavsiyalar", accent: '#ef4444', depth: 5, x: -146, y: -84, floatAmp: 5, floatDur: 7.0, floatDelay: 0.5, pulseDelay: 0.3 },
]
...
function FloatingScene({ mouseX, mouseY, onNavigate, stats, statsLoading }: {
  mouseX: any; mouseY: any; onNavigate: (href: string) => void; stats: any; statsLoading: boolean
}) {
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      <motion.div
        ...
        style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.08) 0%, rgba(59,130,246,0.04) 35%, transparent 65%)' }}
      ...
      <div className="relative w-[460px] h-[480px]">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="-230 -240 460 480" style={{ overflow: 'visible' }}>
          <defs>
          ...
          {ECOSYSTEM_NODES.map((node, i) => (
            <motion.circle
              ...
              transition={{ duration: 3, delay: node.pulseDelay, repeat: Infinity, ease: 'easeInOut' }}
            />
          ))}
        </svg>
        ...
        {ECOSYSTEM_NODES.map((node, i) => {
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0, transition: { delay: 0.3 + i * 0.12, duration: 0.6, ease: 'easeOut' } }}
            >
              <FloatingCard depth={node.depth} index={i} mouseX={mouseX} mouseY={mouseY}>
                <button onClick={() => {
                  const rm: Record<string, string> = {
                    court: '/case-solver', gavel: '/dashboard', documents: '/document-generator',
                  ...
                  <div className="flex items-start gap-2.5">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 shadow-lg"
                      style={{ background: `linear-gradient(135deg, ${node.accent}55, ${node.accent}22)`, color: node.accent }}>
                      {node.icon}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-[11px] font-semibold text-white/90 leading-tight">{node.title}</h3>
                      <p className="text-[9px] text-white/40 mt-0.5 leading-relaxed line-clamp-2">{node.description}</p>
                    </div>
                  </div>
                </button>
              </FloatingCard>
        ...
        >
          ...
          <motion.div
            ...
            style={{
              width: 130, height: 130, left: -27, top: -27,
          ...
          />
          {/* Crystal orb */}
          <div className="relative rounded-2xl backdrop-blur-xl border border-white/15 shadow-2xl flex items-center justify-center overflow-hidden"
            style={{
              width: 76, height: 76,
              background: 'linear-gradient(145deg, rgba(255,255,255,0.12) 0%, rgba(255,255,255,0.02) 50%, rgba(99,102,241,0.08) 100%)',
              boxShadow: '0 0 30px rgba(99,102,241,0.08), 0 8px 40px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.2)',
            }}
          >
            {/* Inner reflections */}
            ...
            {/* Legal AI icon */}
            <svg viewBox="0 0 28 28" className="w-7 h-7 relative z-10" fill="none" stroke="rgba(255,255,255,0.85)" strokeWidth="1.5">
...
function SignInContent() {
  ...
  useEffect(() => {
    firebaseAuth.handleRedirectResult().then(result => {
      if (result.success && result.data) {
        ...
        router.push(eNorm === 'akmaljaxonkulov00@gmail.com' ? '/admin' : (searchParams.get('redirectTo') || '/dashboard'))
      }
    }).catch(() => {})
  }, [router, searchParams])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        } else { setError(result.error || "Email yoki parol noto'g'ri") }
      } else {
        ...
        if (result.success) { setSuccessMsg("Ro'yxatdan o'tish muvaffaqiyatli!"); setTimeout(() => router.replace('/dashboard'), 1500) }
        else { setError(result.error || "Ro'yxatdan o'tish xatosi") }
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    } catch { setError('Google orqali kirishda xatolik') }
    ...
    <div className="min-h-screen lg:grid lg:grid-cols-2" onMouseMove={handleMouseMove}>
      ...
      >
        ...
        <div className="absolute inset-0">
          <FloatingScene mouseX={mouseX} mouseY={mouseY} onNavigate={(h) => router.push(h)} stats={liveStats} statsLoading={statsLoading} />
        </div>
        ...
        >
          <div className="text-center">
            <div className="text-base font-bold text-white/90">{statsLoading ? '—' : <AnimatedCounter value={liveStats.total_users} suffix="+" compact stiffness={90} damping={20} />}</div>
            <div className="text-blue-200/40 text-[9px] tracking-wider uppercase">Foydalanuvchilar</div>
          </div>
          <div className="w-px h-6 bg-white/6" />
          <div className="text-center">
            <div className="text-base font-bold text-white/90">{statsLoading ? '—' : <AnimatedCounter value={liveStats.total_codes} stiffness={90} damping={20} />}</div>
            <div className="text-blue-200/40 text-[9px] tracking-wider uppercase">Kodekslar</div>
          </div>
          <div className="w-px h-6 bg-white/6" />
          <div className="text-center">
            <div className="text-base font-bold text-white/90">{statsLoading ? '—' : <AnimatedCounter value={liveStats.total_ai_requests} suffix="+" compact stiffness={90} damping={20} />}</div>
            <div className="text-blue-200/40 text-[9px] tracking-wider uppercase">AI So'rov</div>
          </div>
          <div className="w-px h-6 bg-white/6" />
          <div className="text-center">
            <div className="text-base font-bold text-white/90">{statsLoading ? '—' : <AnimatedCounter value={liveStats.total_documents} suffix="+" compact stiffness={90} damping={20} />}</div>
      ...
      >
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="lg:hidden text-center mb-6">
            ...
            >
              <svg viewBox="0 0 28 28" className="w-7 h-7 text-white" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M14 4a3 3 0 013 3c0 1.5-1.5 2.25-1.5 2.25S14 8.5 14 7a3 3 0 013-3z" />
                <path d="M8 16a3 3 0 013 3c0 1.5-1.5 2.25-1.5 2.25S8 20.5 8 19a3 3 0 013-3z" />
                <path d="M20 16a3 3 0 013 3c0 1.5-1.5 2.25-1.5 2.25S20 20.5 20 19a3 3 0 013-3z" />
                <circle cx="14" cy="5.5" r="1.5" fill="rgba(255,255,255,0.3)" />
            ...
            <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.4 }}
              className="text-2xl font-bold text-gray-900">JURISAI</motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3, duration: 0.4 }}
              className="text-xs text-gray-500 mt-0.5">Huquqiy AI Platformasi</motion.p>
          </div>

          {/* Premium Glass Form Card */}
          <div className="relative">
            <motion.div className="absolute -inset-1 rounded-2xl blur-xl"
              style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(59,130,246,0.06), rgba(99,102,241,0.08))' }}
            ...
            <div className="relative p-6 sm:p-7 rounded-2xl bg-white/80 backdrop-blur-xl border border-gray-200/60 shadow-xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {mode === 'register' && (
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1.5">Ismingiz</label>
                ...
                {mode === 'login' && (
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={handleForgotPassword} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">
              ...
              <div className="mt-5">
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs"><span className="px-3 bg-white text-gray-400">yoki</span></div>
                </div>
                <motion.button onClick={handleGoogleLogin} disabled={isGoogleLoading}
                  whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
          ...
          >
            ...
            <a href="/terms" className="text-indigo-500 hover:text-indigo-600 underline">Xizmat ko'rsatish shartlari</a>
            {' '}va{' '}
            <a href="/privacy" className="text-indigo-500 hover:text-indigo-600 underline">Maxfiylik siyosati</a>
