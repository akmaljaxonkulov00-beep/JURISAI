...
export default function AIChat() {
  ...
  const handleSendMessage = async () => {
    ...
    // Add user message
    const userMessage = await chatService.addMessage(conversationId, user.id, inputValue, 'user')
            ...
            {messages.length === 0 && !activeConversationId ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-6">
                  Huquqiy savollaringizni bering, professional maslahat oling
                </p>
                ...
                {showSuggestions && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                    {quickActions.map(action => (
                      ...
                      >
                        <div
                          className={`p-3 rounded-lg bg-${action.color}-100 text-${action.color}-600 mb-3 mx-auto w-fit`}
                        >
                          {action.icon}
                        </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        {message.category && (
                          <div className="mt-2">
                            <span
                              className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(message.category)}`}
                            >
                              {message.category}
                            </span>
                          </div>
