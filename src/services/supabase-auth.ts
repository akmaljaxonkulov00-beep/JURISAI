...
/** Supabase User obyektining bizga kerakli qismi */
...
/** Xato obyektidan matn ajratib oladi (unknown catch uchun) */
...
function mapSupabaseUser(sbUser: SupabaseUserLike): AuthUser {
  ...
  return {
    ...
    name:
      ...
      'Foydalanuvchi',
    role: normalizeRole(String(meta.role || 'USER')),
    subscription_plan: String(meta.subscription_plan || 'free'),
    subscription_expires_at: meta.subscription_expires_at
      ? String(meta.subscription_expires_at)
      : undefined,
    avatar: sbUser.avatar || (meta.avatar ? String(meta.avatar) : undefined) || undefined,
    phone: sbUser.phone || (meta.phone ? String(meta.phone) : undefined) || undefined,
    provider,
  }
}
 ...
 */
export async function finalizeUserSession(sbUser: SupabaseUserLike): Promise<AuthUser> {
...
function saveUserToLocal(user: AuthUser) {
  ...
  // tekshiruv (requireUser/requireAdmin) shu token orqali identity oladi.
  // Cookie o'rnatilgandan KEYIN sync-user chaqiriladi (race yo'q).
  syncSessionCookies()
    .then(() => syncUserToSupabase(userWithMeta))
    .catch(() => syncUserToSupabase(userWithMeta))

  // Append to registered_users list for admin
  try {
    const stored = localStorage.getItem('registered_users')
    const users = (stored ? JSON.parse(stored) : []) as Array<
      Partial<AuthUser> & { uid?: string; [key: string]: unknown }
    >
    const existingIdx = users.findIndex(u => u.id === user.id || u.uid === user.id)
    if (existingIdx >= 0) {
      users[existingIdx] = {
        ...users[existingIdx],
        ...userWithMeta,
...
async function syncUserToSupabase(user: AuthUser): Promise<void> {
  try {
    // Authorization header orqali token yuboriladi — server-side identity
    // tekshiruvi uchun (cookie race'ga bog'liq emas).
...
> {
  try {
    // ── Akkaunt yaratish + email tasdiqlash SERVER TOMONIDA ──
    // `/api/auth/signup` emailni service-role bilan bir vaqtning o'zida
    // tasdiqlaydi — alohida public auto-confirm endpoint yo'q (boshqa
    // userlarning emailini tasdiqlab bo'lmaydi).
    ...
    if (!res.ok) {
      return { success: false, error: json?.error || "Ro'yxatdan o'tish xatosi" }
    }

    // Server akkauntni yaratdi va emailni tasdiqladi — endi foydalanuvchini
    // darhol tizimga kiritamiz (session ochamiz).
    const { data: signedIn, error: signInError } = await supabase.auth.signInWithPassword({
      email,
      password,
    })
    if (signInError) {
      // Session ochilmadi (masalan email tasdiqlanmagan) — foydalanuvchiga
      // xabar ko'rsatamiz, emailni kutish shart emas, qayta urinishda ishlaydi.
      return {
        success: true,
        needsEmailConfirmation: true,
        data: { id: '', email, name, role: 'USER' as const },
      }
    }
    if (!signedIn?.user) {
...
export async function signInWithGoogle(): Promise<{
  ...
  try {
    ...
    return { success: false, error: errorText(error) || 'Google orqali kirishda xatolik yuz berdi' }
  }
}
...
export async function handleRedirectResult(): Promise<{
  ...
  try {
    ...
    return { success: false, error: errorText(error) || "Qayta yo'naltirish xatosi" }
  }
}
...
export async function resetPassword(email: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (errorText(error).includes('not found')) {
...
export async function updateProfile(
  ...
  try {
    const userToUpdate: Record<string, string | undefined> = {}
    if (updates.name) {
    ...
    return { success: false, error: errorText(error) || 'Profilni yangilash xatosi' }
  }
}
...
export async function changePassword(
  ...
  try {
    ...
    return { success: false, error: errorText(error) || "Parolni o'zgartirish xatosi" }
  }
}
...
export async function changeEmail(
  ...
  try {
    ...
    if (errorText(error).includes('already')) {
      message = "Bu email allaqachon ro'yxatdan o'tgan"
    }
    return { success: false, error: message }
  }
...
export function onAuthChange(callback: (user: AuthUser | null) => void): () => void {
  ...
  const {
    ...
    // Session tokenlarini cookie'ga sinxron qilamiz (server-side tekshiruv uchun)
