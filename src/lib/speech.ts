...
// Lazy singletons — faqat browserda yaratiladi (SSR safe)
let _tts: AdvancedTTS | null = null;
let _stt: AdvancedSTT | null = null;

export const getTTS = (): AdvancedTTS => {
  if (!_tts) _tts = new AdvancedTTS();
  return _tts;
};

export const getSTT = (): AdvancedSTT => {
  if (!_stt) _stt = new AdvancedSTT();
  return _stt;
};

// Legacy exports for backward compat (safe on server — just empty objects)
export const tts = {
  speak: (text: string, options?: TTSOptions) => isBrowser ? getTTS().speak(text, options || {}) : Promise.resolve(),
  stop:  () => isBrowser && getTTS().stop(),
  pause: () => isBrowser && getTTS().pause(),
  resume:() => isBrowser && getTTS().resume(),
  isSpeaking: () => isBrowser ? getTTS().isSpeaking() : false,
  getVoices: () => isBrowser ? getTTS().getVoices() : [],
  getRussianVoice: () => isBrowser ? getTTS().getRussianVoice() : undefined,
};

export const stt = {
  start:          (options?: STTOptions) => isBrowser && getSTT().start(options || {}),
  startListening: (options?: STTOptions) => isBrowser && getSTT().startListening(options || {}),
  stop:           () => isBrowser && getSTT().stop(),
  stopListening:  () => isBrowser && getSTT().stopListening(),
  abort:          () => isBrowser && getSTT().abort(),
  isListening:    () => isBrowser ? getSTT().isListening() : false,
  getFinalTranscript:   () => isBrowser ? getSTT().getFinalTranscript() : '',
  getInterimTranscript: () => isBrowser ? getSTT().getInterimTranscript() : '',
  clearTranscript:      () => isBrowser && getSTT().clearTranscript(),
};
