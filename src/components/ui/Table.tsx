...
const TableFooter: React.FC<TableFooterProps> = ({ children, className }) => (
  <tfoot className={cn('border-t bg-muted/50 font-medium [&>tr:last-child]:border-b-0', className)}>
    {children}
  </tfoot>
)
...
const TableCaption: React.FC<TableCaptionProps> = ({ children, className }) => (
  <caption className={cn('text-sm text-muted-foreground mt-4', className)}>{children}</caption>
)
