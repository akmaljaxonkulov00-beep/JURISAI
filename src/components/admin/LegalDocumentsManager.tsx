...
export default function LegalDocumentsManager() {
  ...
  const loadCategories = async () => {
    ...
    const { data, error } = await supabaseClient.from('legal_categories').select('*').order('name')
    ...
    try {
      const newDocument = {
        ...
        keywords: formData.keywords
          ...
          .map(k => k.trim())
          .filter(k => k),
        cross_references: formData.cross_references
          .split(',')
          .map(k => k.trim())
          .filter(k => k),
      ...
      const { error } = await supabaseClient.from('legal_documents').insert(newDocument)
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-zinc-400">
            Qonun hujjatlarini qo'shish, tahrirlash va o'chirish
          </p>
        </div>
        <Button
          onClick={() => {
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <Badge className="bg-blue-100 text-blue-800">{document.article_number}</Badge>
                    </div>
                    <p className="text-sm text-gray-600 dark:text-zinc-400 mb-2 line-clamp-2">
                      {document.content.substring(0, 200)}...
                    </p>
                    <div className="flex gap-2">
                      <Badge className="bg-green-100 text-green-800">{document.category}</Badge>
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => openEditModal(document)}>
