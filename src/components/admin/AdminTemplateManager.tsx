...
export default function AdminTemplateManager() {
  ...
  const loadTemplates = (isInitial = false) => {
    ...
    fetch('/api/templates', { credentials: 'include' })
      ...
      .catch(() => {
        if (isInitial) setError("Ma'lumotlarni yuklashda xatolik")
      })
      .finally(() => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">
            Hujjat namunalarini boshqarish
          </h2>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  value={form.lawRef}
                  onChange={e => setForm(f => ({ ...f, lawRef: e.target.value }))}
                  placeholder="Masalan: FPK 103-moddasi"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">
                ...
                <select
                  value={form.format}
                  onChange={e =>
                    setForm(f => ({ ...f, format: e.target.value as 'TXT' | 'DOCX' | 'PDF' }))
                  }
                  className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">
                Teg'lar (vergul bilan ajrating)
              </label>
              <Input
                value={form.tags}
                onChange={e => setForm(f => ({ ...f, tags: e.target.value }))}
                placeholder="huquq, shartnoma, da'vo"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-zinc-700">
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300">
                  Nomi
                </th>
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300 hidden md:table-cell">
                  ID
                </th>
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300">
                  Kategoriya
                </th>
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300 hidden sm:table-cell">
                  Format
                </th>
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300 hidden lg:table-cell">
                  Yuklama
                </th>
                <th className="text-left px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300 hidden lg:table-cell">
                  Sana
                </th>
                <th className="text-right px-3 py-2 font-semibold text-gray-700 dark:text-zinc-300">
                  Harakatlar
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredTemplates.map(t => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-[10px] text-gray-400 dark:text-zinc-500 font-mono hidden md:table-cell">
                    {t.id}
                  </td>
                  <td className="px-3 py-2.5">
                  ...
                  <td className="px-3 py-2.5 hidden sm:table-cell">
                    <Badge className="text-[10px] bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                      {t.format}
                    </Badge>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-gray-500 dark:text-zinc-400 hidden lg:table-cell">
                    {t.downloads}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-gray-500 dark:text-zinc-400 hidden lg:table-cell">
      ...
      {previewContent && (
        ...
        >
          <div
            ...
            className="w-full max-w-3xl max-h-[85vh] overflow-y-auto bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-800"
          >
