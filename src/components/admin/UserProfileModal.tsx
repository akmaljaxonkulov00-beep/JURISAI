...
export default function UserProfileModal({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'payments' | 'usage' | 'activity'>(
    'overview'
  )
  const [paymentFilter, setPaymentFilter] = useState<'all' | 'approved' | 'pending' | 'rejected'>(
    'all'
  )
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="px-2 py-0.5 bg-gray-100 dark:bg-zinc-800 text-gray-700 dark:text-zinc-300 rounded-full text-xs">
            {status}
          </span>
        )
    }
      ...
      <div className="relative w-full max-w-4xl mx-4 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-800 overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-800/30">
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-zinc-100">
                        {details.paymentStats.total}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                        {details.paymentStats.approved} tasdiqlangan ·{' '}
                        {details.paymentStats.totalAmount.toLocaleString('uz-UZ')} so'm
                      </p>
                    </div>
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4 border border-purple-100 dark:border-purple-800/30">
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-zinc-100">
                        {details.usageStats.total}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                        {details.usageStats.chatQueries} chat · {details.usageStats.documents}{' '}
                        hujjat
                      </p>
                    </div>
                    <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl p-4 border border-green-100 dark:border-green-800/30">
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-zinc-100">
                        {details.loginActivity.loginCount || 0}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-zinc-400 mt-1">
                  ...
                  {!details.profile && (
                    <div className="text-center py-8 text-gray-500 dark:text-zinc-400">
                      ...
                      <p className="text-xs mt-1">
                        Foydalanuvchi auth.users da mavjud, lekin registered_users jadvalida yo'q
                      </p>
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-500 dark:text-zinc-400">
                        Jami: {details.paymentStats.total} ta ·{' '}
                        {details.paymentStats.totalAmount.toLocaleString('uz-UZ')} so'm
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {filteredPayments.map(p => {
                        ...
                        return (
                          <div
                            key={p.id}
                            className="p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
                          >
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {p.notes && (
                                  <span
                                    className="text-xs text-gray-400 dark:text-zinc-500 max-w-[100px] truncate"
                                    title={p.notes}
                                  >
                                    {p.notes}
                                  </span>
                  ...
                  {details.usageLogs.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                      {details.usageLogs.map(log => (
                        <div
                          key={log.id}
                          className="flex items-center justify-between p-3 bg-gray-50 dark:bg-zinc-800/50 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div
                              className={`w-2 h-2 rounded-full flex-shrink-0 ${
                                log.action?.toLowerCase().includes('chat')
                                  ? 'bg-blue-500'
                                  : log.action?.toLowerCase().includes('document')
                                    ? 'bg-purple-500'
                                    : log.action?.toLowerCase().includes('analyz')
                                      ? 'bg-orange-500'
                                      : 'bg-gray-400'
                              }`}
                            />
                        ...
                        <div key={`pay-${p.id}`} className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm text-gray-900 dark:text-zinc-100">
                              To'lov: {p.amount.toLocaleString('uz-UZ')} so'm —{' '}
                              {p.status === 'approved'
                                ? 'Tasdiqlandi'
                                : p.status === 'pending'
                                  ? 'Kutilmoqda'
        ...
        <div className="px-6 py-3 border-t border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-4 py-1.5 text-sm text-gray-600 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-lg transition-colors"
            >
              Yopish
            </button>
            <button
              onClick={fetchDetails}
              className="px-4 py-1.5 text-sm text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
            >
              Yangilash
            </button>
          </div>
