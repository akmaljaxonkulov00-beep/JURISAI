...
const WeaknessDetector: React.FC = () => {
  ...
  const [argumentType, setArgumentType] = useState<
    'irac_analysis' | 'legal_brief' | 'court_argument' | 'contract_claim'
  >('irac_analysis')
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const mockAnalysis: WeaknessAnalysisResult = {
        ...
        overall_score: 72,
        strength_areas: ['Mantiqiy tuzilma', 'Aniq muammoni belgilash', 'Xulosa aniqligi'],
        ...
        detailed_feedback:
          "Sizning argumentlaringiz yaxshi tuzilgan va aniq muammoni ko'rsatadi. Biroq, dalillar bazasi kuchaytirilishi va qonunni to'g'ri qo'llash tavsiya etiladi. Xissiy so'zlardan qochinib, ob'ektiv tilda yozish argumentlaringizning ishonchini oshiradi.",
        processing_time: 2.3,
        timestamp: new Date(),
      }
  ...
  const getWeaknessTypeName = (type: string) => {
    switch (type) {
      case 'logical_fallacy':
        return 'Mantiqiy xato'
      case 'insufficient_evidence':
        return 'Dalillar yetarli emas'
      case 'irrelevant_argument':
        return 'Aloqasiz argument'
      case 'legal_misapplication':
        return "Qonunni noto'g'ri qo'llash"
      case 'structural_issue':
        return 'Tuzilma muammosi'
      case 'emotional_reasoning':
        return 'Hissiy mulohazalar'
      case 'missing_precedent':
        return "Pretsedent yo'q"
      case 'unclear_conclusion':
        return "Noma'lum xulosa"
      default:
        return type
    }
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-600 dark:text-zinc-400">
          Huquqiy argumentlardagi kamchiliklarni aniqlash
        </p>
      </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium capitalize">
                        {analysis.argument_type.replace('_', ' ')}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                    ...
                    <div className="text-center">
                      <span
                        className={cn('text-4xl font-bold', getScoreColor(analysis.overall_score))}
                      >
                      ...
                      <div key={index} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="font-medium">
                              {getWeaknessTypeName(weakness.type)}
                            </span>
                          </div>
                          <span className="text-sm text-gray-500 dark:text-zinc-500">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-500 dark:text-zinc-500">
                            Joylashuv: {weakness.location}
                          </span>
                          <span className="text-gray-500 dark:text-zinc-500">
                            Ta'sir: {Math.round(weakness.impact_score * 100)}%
                          </span>
                        </div>
