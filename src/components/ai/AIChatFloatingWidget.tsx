  ...
  return lines.map((line, li) => {
    ...
    if (l.startsWith('>')) {
      return (
        <blockquote
          key={li}
          className="pl-3 border-l-2 text-sm italic leading-relaxed opacity-85"
          style={{ borderColor: accentColor, color: darkFg }}
        >
    ...
    if (l.startsWith('•')) {
      return (
        <div key={li} className="flex gap-1.5 text-sm leading-relaxed" style={{ color: darkFg }}>
          <span className="flex-shrink-0 select-none" style={{ color: accentColor }}>
            •
          </span>
    ...
    // Regular paragraph
    return (
      <p key={li} className="text-sm leading-relaxed" style={{ color: darkFg }}>
        {renderBold(l)}
      </p>
    )
  })
  ...
  return blocks.map((block, bi) => {
    ...
    if (headerEmoji) {
      ...
      return (
        ...
        >
          <h3
            className="px-3.5 py-2 text-xs font-bold tracking-wide m-0"
    ...
    if (trimmed.startsWith('>')) {
      ...
      return (
        <div
          key={bi}
          className="my-2 pl-3 border-l-2 border-blue-400 dark:border-blue-500 bg-blue-50/50 dark:bg-blue-900/10 rounded-r-lg py-1.5 pr-3"
        >
          {lines.map((line, li) => {
            ...
            return (
              <p
                key={li}
                className="text-sm text-gray-700 dark:text-zinc-300 italic leading-relaxed"
              >
                {renderBold(t)}
              </p>
            )
        ...
        <div key={bi} className="space-y-1 my-2">
          {lines.map((line, li) => {
            ...
            return (
              <div
                key={li}
                className="flex gap-1.5 text-sm text-gray-700 dark:text-zinc-300 leading-relaxed"
              >
...
export default function AIChatFloatingWidget() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const assistantText =
        data.answer || data.response || 'Kechirasiz, javob olishda xatolik yuz berdi.'
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`fixed z-50 overflow-hidden flex flex-col backdrop-blur-xl bg-white/95 dark:bg-zinc-900/95 animate-in slide-in-from-bottom-4 duration-300 ${
            fullScreen
              ...
              <div className="p-3 space-y-2">
                ...
                {sessions.length === 0 ? (
                  ...
                  groupSessions(sessions).map(group => (
                    <div key={group.label}>
                      <p className="text-[10px] font-medium text-gray-400 dark:text-zinc-500 px-1 mb-1">
                        {group.label}
                      </p>
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <div className="flex flex-wrap gap-2 mt-4 justify-center">
                  {['Shartnoma tuzish', 'Ish haqi', 'Ajralish', 'Meros', 'Jinoyat kodeksi'].map(
                    q => (
                      <button
                        key={q}
                        onClick={() => setInput(q)}
              ...
              messages.map(msg => (
                ...
                >
                  ...
                  >
                    ...
                    <p
                      className={`text-[10px] mt-1 ${msg.role === 'user' ? 'text-blue-200' : 'text-gray-400 dark:text-zinc-500'}`}
                    >
              ...
              <div className="flex justify-start">
                <div className="bg-gray-100 dark:bg-zinc-800 px-3.5 py-3 rounded-2xl rounded-bl-md">
                  <div className="flex gap-1.5">
                    <div
                      className="w-2 h-2 bg-blue-50 dark:bg-blue-900/20 rounded-full animate-bounce"
