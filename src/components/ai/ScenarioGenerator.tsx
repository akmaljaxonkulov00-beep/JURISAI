...
const ScenarioGenerator: React.FC = () => {
  ...
  const generateScenario = async (req: ScenarioGenerationRequest): Promise<LegalScenario> => {
    ...
    const scenarioData = {
      contract_dispute: {
        ...
        background:
          "A kompaniyasi B kompaniyasidan marketing xizmatlarini olgan, ammo to'lov muddati kechikishi sababli shartnoma buzilgan.",
        facts: [
      ...
      property_dispute: {
        ...
        background:
          'Ikki aka-uka ota meros qoldan uy-joyga egalik huquqini belgilash uchun sudga murojaat qilishdi.',
        facts: [
    ...
    const typeData =
      scenarioData[req.scenario_type as keyof typeof scenarioData] || scenarioData.contract_dispute
  ...
  const getTypeName = (type: string) => {
    switch (type) {
      case 'contract_dispute':
        return 'Shartnoma nizolari'
      case 'property_dispute':
        return 'Mulk nizolari'
      case 'tort_claim':
        return "Tort da'volari"
      case 'family_law':
        return 'Oila huquqi'
      case 'criminal_case':
        return 'Jinoyat ishlari'
      case 'administrative_law':
        return "Ma'muriy huquq"
      case 'business_law':
        return 'Biznes huquqi'
      case 'employment_law':
        return 'Mehnat huquqi'
      default:
        return type
    }
  }
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setRequest(prev => ({ ...prev, difficulty_level: e.target.value as any }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-zinc-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setRequest(prev => ({ ...prev, complexity: e.target.value as any }))
                    }
                    className="w-full px-3 py-2 border border-gray-300 dark:border-zinc-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setRequest(prev => ({ ...prev, estimated_time: parseInt(e.target.value) }))
                  }
                />
              </div>
