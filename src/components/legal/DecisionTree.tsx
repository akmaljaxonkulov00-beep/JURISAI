...
const DecisionTree: React.FC = () => {
      ...
      options: [
        {
          ...
          text: 'Xulosa',
          consequence:
            "Kechikish qonuniy asoslanganligi uchun tovon puli kamaytirilishi yoki to'lanmasligi mumkin",
          probability: 0.2,
          legal_basis: 'Fuqarolik kodeksi 367-moddasi 3-qism',
        },
      ],
      ...
      options: [
        {
          ...
          text: 'Xulosa',
          consequence:
            "Jiddiy kechikish uchun foizlar, asosiy summa va qo'shimcha zararni to'lash shart",
          probability: 0.95,
          legal_basis: 'Fuqarolik kodeksi 367-moddasi 2-qism',
        },
      ],
  ...
  const getCurrentNode = (): TreeNode => {
    return (
      decisionTrees[currentNodeId] ||
      decisionTrees[selectedScenario] ||
      decisionTrees.contract_dispute
    )
  }
  ...
  const analyzeDecisionPath = async (path: DecisionPath[]) => {
    ...
    try {
      ...
      const mockResult: AnalysisResult = {
        final_decision:
          "Shartnoma buzilishi uchun 30 million so'm miqdorida tovon puli to'lash tavsiya etiladi",
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-600 dark:text-zinc-400">
          Huquqiy vaziyatlarda qaror qabul qilish yo\'nalishini aniqlang
        </p>
      </div>
                    ...
                    {getCurrentNode().options.map(option => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {option.probability && (
                            <Badge variant="outline">{Math.round(option.probability * 100)}%</Badge>
                          )}
                        </div>
                        {option.legal_basis && (
            ...
            <Card>
              <CardContent className="flex items-center justify-center py-12">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-gray-600 dark:text-zinc-400">
                    Qaror yo\'nalishi tahlil qilinmoqda...
                  </p>
                  <p className="text-sm text-gray-500 dark:text-zinc-500">
                    ...
                    <div className="text-center">
                      ...
                      <div
                        className={cn(
                          ...
                          getConfidenceColor(analysisResult.confidence_score)
                        )}
                      >
                    ...
                    {analysisResult.alternative_paths.map((path, index) => (
                      <div key={index} className="border rounded-lg p-4">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Badge variant="outline">{Math.round(path.probability * 100)}%</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
