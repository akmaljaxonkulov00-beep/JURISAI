...
function makeFallback(id: string, name: string): LegalCode {
  return { id, name, shortName: name, description: '', totalArticles: 0, effectiveDate: '01.01.2024', articles: [] };
}

// ── Code exports ─────────────────────────────────────────────────
// Each tries JSON first, falls back to minimal empty stub

export const CRIMINAL_CODE: LegalCode = FULL_CODES.find(c => c.id === 'criminal_code') || makeFallback('criminal_code', "O'zbekiston Respublikasi Jinoyat Kodeksi");
export const CIVIL_CODE: LegalCode = FULL_CODES.find(c => c.id === 'civil_code') || makeFallback('civil_code', "O'zbekiston Respublikasi Fuqarolik Kodeksi");
export const LABOR_CODE: LegalCode = FULL_CODES.find(c => c.id === 'labor_code') || makeFallback('labor_code', "O'zbekiston Respublikasi Mehnat Kodeksi");
export const FAMILY_CODE: LegalCode = FULL_CODES.find(c => c.id === 'family_code') || makeFallback('family_code', "O'zbekiston Respublikasi Oila Kodeksi");
export const TAX_CODE: LegalCode = FULL_CODES.find(c => c.id === 'tax_code') || makeFallback('tax_code', "O'zbekiston Respublikasi Soliq Kodeksi");
export const LAND_CODE: LegalCode = FULL_CODES.find(c => c.id === 'land_code') || makeFallback('land_code', "O'zbekiston Respublikasi Yer Kodeksi");
export const ADMINISTRATIVE_CODE: LegalCode = FULL_CODES.find(c => c.id === 'admin_code') || makeFallback('admin_code', "O'zbekiston Respublikasi Ma'muriy javobgarlik to'g'risidagi Kodeksi");
export const CONSTITUTION: LegalCode = FULL_CODES.find(c => c.id === 'constitution') || makeFallback('constitution', "O'zbekiston Respublikasi Konstitutsiyasi");
export const CIVIL_PROCEDURE_CODE: LegalCode = FULL_CODES.find(c => c.id === 'civil_procedure_code') || makeFallback('civil_procedure_code', "O'zbekiston Respublikasi Fuqarolik protsessual Kodeksi");
export const CRIMINAL_PROCEDURE_CODE: LegalCode = FULL_CODES.find(c => c.id === 'criminal_procedure_code') || makeFallback('criminal_procedure_code', "O'zbekiston Respublikasi Jinoyat-protsessual Kodeksi");
export const ECONOMIC_PROCEDURE_CODE: LegalCode = FULL_CODES.find(c => c.id === 'economic_procedure_code') || makeFallback('economic_procedure_code', "O'zbekiston Respublikasi Iqtisodiy protsessual Kodeksi");

// ── Aggregate exports ────────────────────────────────────────────

export const ALL_LEGAL_CODES: LegalCode[] = [
  CRIMINAL_CODE, CIVIL_CODE, LABOR_CODE, FAMILY_CODE,
  TAX_CODE, LAND_CODE, ADMINISTRATIVE_CODE,
...
export function getArticleByNumber(codeId: string, articleNumber: string): LegalArticle | undefined {
