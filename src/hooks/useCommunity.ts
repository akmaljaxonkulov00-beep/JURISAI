...
function getCurrentUser(): CommunityUser {
  if (typeof window === 'undefined') {
    return { id: 'guest', name: 'Mehmon', avatar: 'user', role: 'Foydalanuvchi', verified: false, reputation: 0 };
  }
  try {
    const stored = localStorage.getItem(LS_CURRENT_USER) || sessionStorage.getItem('jurisai_user') || localStorage.getItem('auth_user');
    if (stored) {
  ...
  return { id: 'guest_' + Date.now(), name: 'Mehmon', avatar: 'user', role: 'Foydalanuvchi', verified: false, reputation: 0 };
}
...
export function useCommunity() {
  ...
  const createPost = useCallback((content: string, category: CommunityPost['category'], tags: string[]) => {
  ...
  const updatePost = useCallback((postId: string, content: string, tags: string[], category?: CommunityPost['category']) => {
    const updated = posts.map(p => {
      if (p.id === postId) {
        return { ...p, content, tags, category: category || p.category, updatedAt: new Date().toISOString() };
      }
      return p;
    });
    setPosts(updated);
  ...
  const deleteComment = useCallback((postId: string, commentId: string) => {
    const updated = posts.map(p => {
      ...
      const removeRecursive = (comments: CommunityComment[]): CommunityComment[] =>
        comments.filter(c => c.id !== commentId).map(c => ({ ...c, replies: removeRecursive(c.replies) }));
  ...
  const addNotification = (type: CommunityNotification['type'], message: string, fromUser: string, postId?: string) => {
    ...
    setNotifications(prev => {
      ...
      try { localStorage.setItem(LS_NOTIFICATIONS, JSON.stringify(updated)); } catch {}
      return updated;
    });
  };
  ...
  const clearNotifications = useCallback(() => {
    setNotifications([]);
    try { localStorage.removeItem(LS_NOTIFICATIONS); } catch {}
  }, []);
